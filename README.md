...
- [ ] Технический долг
    - [ ] Отрефакторить подмену значений из конфигов/шаблонов. Из конфига
      извлекается с помощью `callback=`, а из шаблона с помощью кастомного
      класса `Option`. Возможно, стоит переехать на кастомный `type=` или но
      callback явно не для этого, т.к. вызывается тогда, когда параметр уже
      обработал и требует дублировать, например, логику `required=`
